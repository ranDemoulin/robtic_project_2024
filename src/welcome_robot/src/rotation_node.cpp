...
void compute_rotation()
{
    ...
    // 
    error_rotation =  rotation_to_do - rotation_done; // like remaining rotation to do
    ...
    //cond_rotation is used to control if we stop or not the pid i.e when we considerthe rotation is finished. 
    // Take care that rotation_to_do could be negative
    cond_rotation = fabs(error_rotation) > error_rotation_threshold; 
    // cond_rotation = !(rotation_to_do - error_rotation_threshold < rotation_done && rotation_done < rotation_to_do + error_rotation_threshold); 
