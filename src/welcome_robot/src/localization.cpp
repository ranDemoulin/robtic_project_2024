...
void localization::initialize_localization()
{
    ...
    //we search the position with the highest sensor_model in a square of 2x2 meters
    // around the initial_position and with all possible orientations  
    float min_x = initial_position.x-1, max_x = initial_position.x+1;
    float min_y = initial_position.y-1, max_y = initial_position.y+1;
...
void localization::estimate_position()
{
    ...
    // we search the position with the highest sensor_model in a square of 1x1 meter 
    // around the predicted_position and with orientations around the predicted_orientation -M_PI/6 and +M_PI/6
    float min_x = predicted_position.x-0.5, max_x = predicted_position.x+0.5;
    float min_y = predicted_position.y-0.5, max_y = predicted_position.y+0.5;
    float min_orientation = predicted_orientation - M_PI/6, max_orientation = predicted_orientation + M_PI/6;
    ...
    score_max = sensor_model(predicted_position.x, predicted_position.y, predicted_orientation);
...
void localization::find_best_position(float min_x, float max_x, float min_y, float max_y, float min_orientation, float max_orientation)
{
    ...
    for(float loop_x = min_x; loop_x < max_x; loop_x+=0.05){
        for(float loop_y = min_y; loop_y < max_y; loop_y+=0.05){
            for(float o = min_orientation; o < max_orientation; o+=(5.0/180)*M_PI){
...
int localization::sensor_model(float x, float y, float o)
{
    ...
    {
       //////////////////////////////////////////////////////////////////////////////// // TO COMPLETE
        hit[loop].x = x + r[loop]*cos(theta[loop]+o);
        hit[loop].y = y + r[loop]*sin(theta[loop]+o);
