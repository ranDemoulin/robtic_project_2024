...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // New publisher fior project : tracked person position
    pub_tracked_person = n.advertise<geometry_msgs::Point>("person_position", 1);
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void datmo::detect_motion()
{
    ...
    for (int loop_hit = 0; loop_hit < nb_beams; loop_hit++)
        if(fabs(background[loop_hit] - r[loop_hit]) > detection_threshold){
...
void datmo::perform_basic_clustering()
{
    ...
    for (int loop_hit = 1; loop_hit < nb_beams; loop_hit++)
    {
        if( distancePoints(current_scan[loop_hit-1], current_scan[loop_hit]) > cluster_threshold){
            ...
            cluster_end[current_cluster] = loop_hit-1;
            current_cluster++;
            nb_clusters++;

            // Start new cluster
            cluster_start[current_cluster] = loop_hit;
        }
...
void datmo::perform_advanced_clustering()
{
    ...
    {
        // Definition of required variable 
        int start = cluster_start[loop_cluster];
        int end = cluster_end[loop_cluster];

        // Computation of the cluster size
        cluster_size[loop_cluster] = distancePoints(current_scan[start], current_scan[end]);

        // Computation of the middle point
        cluster_middle[loop_cluster].x = ((current_scan[start].x + current_scan[end].x)/2) ;
        cluster_middle[loop_cluster].y = ((current_scan[start].y + current_scan[end].y)/2) ;
        cluster_middle[loop_cluster].z = ((current_scan[start].z + current_scan[end].z)/2) ;

        // Computation of the percentage of dynamic point in  cluster
        if(end-start != 0){
            cluster_dynamic[loop_cluster] = (int) (((float)(compute_nb_dynamic(start,end))/(float)(end-start))*100);        
        }
       ...
       if( cluster_size[loop] > leg_size_min &&  cluster_size[loop] < leg_size_max)
        {
            ...
            if (cluster_dynamic[loop] >= dynamic_threshold)
            {
                ...
                ROS_INFO("a moving leg is registered");
            }else{
...
void datmo::detect_persons()
{
    ...
    for (int loop_leg_right = 0; loop_leg_right < nb_legs_detected; loop_leg_right++)
        ...
        {
           if(distancePoints(leg_detected[loop_leg_right],leg_detected[loop_leg_left]) < legs_distance_max ){
                ...
                person_detected[nb_persons_detected].z = ((leg_detected[loop_leg_right].z + leg_detected[loop_leg_left].z)/2) ;

                leg_left[nb_persons_detected] = loop_leg_left;
                leg_right[nb_persons_detected] = loop_leg_right;
                
                if(leg_dynamic[leg_left[nb_persons_detected]] && leg_dynamic[leg_right[nb_persons_detected]]){
                    person_dynamic[nb_persons_detected] = true;
                    ROS_INFO("a moving person is registered");
    ...
    ROS_INFO("end detecting persons");
...
void datmo::detect_a_moving_person()
{
    ...
    geometry_msgs::Point pos_robot;
    pos_robot.x = 0;
    pos_robot.y = 0;
    pos_robot.z = 0;

    int person_to_track = 0;
    bool test_person_detected = false;
    for (int loop_persons = 0; loop_persons < nb_persons_detected; loop_persons++){
        if (person_dynamic[loop_persons]){
            if(distancePoints(person_detected[loop_persons],pos_robot) < distancePoints(person_detected[person_to_track],pos_robot) ){
                person_to_track = loop_persons;
            }
            test_person_detected = true;
        }
    }

    if (test_person_detected){    
        person_tracked = person_detected[person_to_track];
...
void datmo::track_a_person()
{
    ...
    for (int loop_persons = 0; loop_persons < nb_persons_detected; loop_persons++){
        ...
        if(fabs(distancePoints(person_detected[loop_persons],person_tracked)) < distance_min){
            index_min = loop_persons;
            distance_min = distancePoints(person_detected[loop_persons],person_tracked);
            associated = true;
        }        
    ...
    if ( associated )
    {
        // update the information related to the person_tracked, frequency increase and uncertainty is set to based val knowing that there is an association
    ...
    {
        // update the information related to the person_tracked, frequency decrease and uncertainty increase knowing that there is no association
        // should we publish or not person_tracked ?
     ...
     ROS_INFO("frequency is %i",frequency);    
